...
# Configuración de página - Sin menús de descarga
st.set_page_config(page_title="Radar Maestro Golden", layout="wide", initial_sidebar_state="collapsed")

# Estilo CSS para ocultar botones de descarga de tablas que Streamlit pone por defecto
st.markdown("""
    <style>
...
def analizar_activo(ticker):
    try:
        ...
        # Gestión Dinámica Real
        tr = pd.concat([hist['High']-hist['Low'], abs(hist['High']-hist['Close'].shift())], axis=1).max(axis=1)
        atr = float(tr.rolling(14).mean().iloc[-1])
        
        sl = cp - (atr * 2)
        tp = float(hist['High'].rolling(20).max().iloc[-1])
        
        riesgo = cp - sl
        beneficio = tp - cp
        ratio_calc = beneficio / riesgo if riesgo > 0 else 0
        
        # NOMENCLATURA MANUAL ESTRICTA
        ...
        return {
            "Mercado": mercado, "Ticker": ticker, "Nombre": info.get('shortName', ticker),
            "Sector": info.get('sector', 'N/A'), "Precio": round(cp, 2), "Media 20": round(sma20, 2),
            "RSI": round(rsi, 2), "ATR": round(atr, 2), "Stop Loss": round(sl, 2),
            "Take Profit": round(tp, 2), "Ratio": round(ratio_calc, 2),
            "Tendencia": tendencia, "Recomendación": rec
        }
    except: return None

if st.button('🚀 EJECUTAR ESCÁNER PROFESIONAL'):
    with st.spinner('Analizando mercado en tiempo real...'):
        ...
        if not df.empty:
            ...
            # Se muestra la tabla pero el CSS de arriba oculta la opción de descargar
