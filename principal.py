...
def analizar_activo(ticker):
    try:
        ...
        sl = cp - (atr * 2) # Riesgo: 2 veces la volatilidad
        tp = float(hist['High'].rolling(10).max().iloc[-1]) # Objetivo: Máximo de los últimos 10 días
        ...
        rec = "COMPRAR 💎" if (cp > sma20 and 30 < rsi < 65 and ratio_real > 1.5) else "ESPERAR ⏳"

        return {
            "Mercado": mercado, "Ticker": ticker, "Nombre": info.get('shortName', ticker),
            "Sector": info.get('sector', 'N/A'), "Precio": round(cp, 2), "RSI": round(rsi, 2),
            "ATR": round(atr, 2), "Stop Loss": round(sl, 2), "Take Profit": round(tp, 2),
            "Ratio": round(ratio_real, 2), "Tendencia": tendencia, "Recomendación": rec
        }
    except: return None

if st.button('🚀 EJECUTAR ANÁLISIS MAESTRO'):
    with st.spinner('Analizando Ratios reales...'):
        todos = USA + ARG
